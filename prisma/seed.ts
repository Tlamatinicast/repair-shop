...
async function main() {
  ...
  await Promise.all([
    prisma.repair.create({ data: { ticketNumber: 'TK-0001', status: 'IN_REPAIR', customerId: customers[0].id, deviceType: 'Laptop', deviceBrand: 'Apple', deviceModel: 'MacBook Pro 13" 2020', issue: 'No enciende, daño por líquido', diagnosis: 'Corrosión en placa lógica', laborCost: 800, totalCost: 1580, createdAt: d(3), parts: { create: [{ itemId: items[2].id, quantity: 1, unitPrice: 180 }, { itemId: items[1].id, quantity: 1, unitPrice: 600 }] } } }),
    prisma.repair.create({ data: { ticketNumber: 'TK-0002', status: 'READY', customerId: customers[1].id, deviceType: 'Consola', deviceBrand: 'Sony', deviceModel: 'PlayStation 5', issue: 'Sobrecalentamiento', diagnosis: 'Ventilador obstruido + pasta térmica degradada', laborCost: 350, totalCost: 1280, createdAt: d(5), parts: { create: [{ itemId: items[4].id, quantity: 1, unitPrice: 750 }, { itemId: items[2].id, quantity: 1, unitPrice: 180 }] } } }),
    prisma.repair.create({ data: { ticketNumber: 'TK-0003', status: 'WAITING_PARTS', customerId: customers[2].id, deviceType: 'GPU', deviceBrand: 'NVIDIA', deviceModel: 'RTX 3060 Ti', issue: 'Artefactos en pantalla', laborCost: 600, totalCost: 600, createdAt: d(1) } }),
    prisma.repair.create({ data: { ticketNumber: 'TK-0004', status: 'RECEIVED', customerId: customers[3].id, deviceType: 'Laptop', deviceBrand: 'Dell', deviceModel: 'XPS 15 9500', issue: 'Pantalla parpadeante', laborCost: 0, totalCost: 0, createdAt: d(0) } }),
    prisma.repair.create({ data: { ticketNumber: 'TK-0005', status: 'DELIVERED', customerId: customers[0].id, deviceType: 'Laptop', deviceBrand: 'Lenovo', deviceModel: 'ThinkPad X1 Carbon', issue: 'Batería no carga', diagnosis: 'Batería agotada', laborCost: 200, totalCost: 1400, createdAt: d(10), deliveredAt: d(7), parts: { create: [{ itemId: items[1].id, quantity: 1, unitPrice: 1200 }] } } }),
  ]);
