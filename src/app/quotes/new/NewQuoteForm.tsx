...
import { ArrowLeft, Plus, Trash2, Save, Search, X, AlertCircle } from 'lucide-react';
...
interface QuoteItem {
  ...
  quantity: number;
  unitPrice: number;
  priceInput: string; // raw input, may be "con IVA"
...
function makeItem(): QuoteItem {
  return {
    ...
    priceInput: '', enterWithIva: false, hasIva: false, onDemand: false,
  };
}
...
export function NewQuoteForm({
  ...
  businessName: string;
  preCustomer: { id: number; name: string; phone: string } | null;
}) {
  ...
  // Customer
  const [useExisting, setUseExisting] = useState(!!preCustomer);
  const [customer, setCustomer] = useState<Customer | null>(preCustomer ?? null);
  ...
  validUntilDefault.setDate(validUntilDefault.getDate() + defaultValidityDays);
  const [validUntil, setValidUntil] = useState(validUntilDefault.toISOString().slice(0, 10));
  ...
  const updateItem = (id: string, patch: Partial<QuoteItem>) => {
    setItems(prev => prev.map(it => {
      ...
      // Recalculate unitPrice when price mode or value changes
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto animate-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="card p-5">
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button
                type="button"
                onClick={() => { setUseExisting(false); setCustomer(null); }}
                className={`text-xs px-3 py-1.5 rounded-md border transition-colors ${!useExisting ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'text-[#666] border-[#222] hover:text-[#ccc]'}`}
              >Sin cuenta</button>
            </div>
            ...
            {useExisting ? (
              customer ? (
                <div className="flex items-center justify-between p-3 bg-[#111] rounded-lg border border-[#1e1e1e]">
                  ...
                  <button onClick={() => { setCustomer(null); setCustomerSearch(''); }} className="text-[#444] hover:text-red-400">
                    <X size={14} />
                  </button>
                </div>
              ) : (
                    ...
                    <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-[#111] border border-[#222] rounded-lg overflow-hidden shadow-xl">
                      ...
                      {searchResults.map(c => (
                        <button
                          ...
                          onClick={() => { setCustomer(c); setSearchResults([]); setCustomerSearch(''); }}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={manualName} onChange={e => setManualName(e.target.value)} className="input" placeholder="Nombre completo" />
                </div>
                <div>
                  <label className="label">Teléfono</label>
                  <input value={manualPhone} onChange={e => setManualPhone(e.target.value)} className="input" placeholder="10 dígitos" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              {items.map((item, idx) => {
                const { subtotal: sub, iva, total: tot } = calcItem(item);
                return (
                  <div key={item.id} className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg p-4 space-y-3">
                    <div className="flex items-start gap-2">
                      ...
                      <button onClick={() => removeItem(item.id)} className="text-[#333] hover:text-red-400 mt-2 flex-shrink-0">
                        <Trash2 size={13} />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 pl-7">
                      ...
                      <div>
                        <label className="label">
                          Precio {item.enterWithIva ? 'c/IVA' : 's/IVA'}
                        </label>
                        <input
                          type="number" min="0" step="0.01"
                          value={item.priceInput}
                          onChange={e => updateItem(item.id, { priceInput: e.target.value })}
                          className="input text-sm"
                          placeholder="0.00"
                        />
                      </div>
                    ...
                    <div className="flex items-center gap-4 pl-7">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox" checked={item.hasIva}
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox" checked={item.enterWithIva}
                          onChange={e => updateItem(item.id, { enterWithIva: e.target.checked, priceInput: '' })}
                          className="w-3.5 h-3.5 accent-amber-500"
                        />
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox" checked={item.onDemand}
                          onChange={e => updateItem(item.id, { onDemand: e.target.checked })}
                          className="w-3.5 h-3.5 accent-amber-500"
                        />
          ...
          <div className="card p-5 space-y-4">
            ...
            <div>
              <label className="label">Notas internas (no aparecen en el PDF)</label>
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              {ivaTotal > 0 && (
                <div className="flex justify-between text-[#555] text-xs">
                  <span>IVA (16%)</span>
                  <span className="font-mono">{fmt(ivaTotal)}</span>
                </div>
              )}
          ...
          <Link href="/quotes" className="btn-secondary w-full justify-center">Cancelar</Link>
        </div>
      </div>
    </div>
  );
